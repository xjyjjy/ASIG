...
class padding_v4_1(object):
    ...
    def get_level_padding_idx(self, level, flag = 0):
        ...
        index_in = np.concatenate((index0_in, index1_in, index2_in, index3_in, index4_in, index5_in, index6_in, index7_in),axis=0)
        index_out = np.concatenate((index0_out, index1_out, index2_out, index3_out, index4_out, index5_out, index6_out, index7_out),axis=0)
        # index_in = np.concatenate((index0_in, index1_in,  index2_in, index3_in,),axis=0)
        # index_out = np.concatenate((index0_out, index1_out, index2_out, index3_out,),axis=0)
        # index_in = np.concatenate((index4_in, index5_in, index6_in, index7_in),axis=0)
        # index_out = np.concatenate((index4_out, index5_out, index6_out, index7_out),axis=0)
        # index_in = np.concatenate((index4_in, index5_in),axis=0)
        # index_out = np.concatenate((index4_out, index5_out),axis=0)
...
class padding_v4_0(object):
    ...
    def get_level_padding_idx(self, level, flag = 0):
        ...
        # self.index_lookup[level][0] = torch.Tensor(index_out).type(torch.long).to(self.device)
        # self.index_lookup[level][1] =  torch.Tensor(index_in).type(torch.long).to(self.device)
        self.index_lookup[level][0] = torch.tensor(index_out, dtype=torch.int32)
        self.index_lookup[level][1] = torch.tensor(index_in,  dtype=torch.int32)

    def get_padding(self, input, level, add=False):
        idx_out = self.index_lookup[level][0].to(input.device)
        idx_in  = self.index_lookup[level][1].to(input.device)
        bs, c, h, w = input.shape
        input = input.view(bs//10, 10, c, h, w)  # reshape to [bs, 10, h, w]
        input_tr = input.permute(0, 1, 3, 4, 2)
        input_tr = input_tr.clone() 
        if not add:
            for i in range(0, 10, 2):
                input_tr[:, (idx_out[:,0]+i)%10, idx_out[:,1],  idx_out[:,2]] = \
                input_tr[:, (idx_in[:,0]+i)%10, idx_in[:,1],  idx_in[:,2]]
        else:
            # print("add")
            input_clone = input_tr.clone()
            for i in range(0, 10, 2):
                input_tr[:, (idx_out[:,0]+i)%10, idx_out[:,1],  idx_out[:,2]] = \
                0.5*input_clone[:, (idx_in[:,0]+i)%10, idx_in[:,1],  idx_in[:,2]] + \
                0.5*input_clone[:, (idx_out[:,0]+i)%10, idx_out[:,1],  idx_out[:,2]]
            del input_clone
                # 0.5*input_clone[:, (self.index_lookup[level][0][:,0]+i)%10, self.index_lookup[level][0][:,1],  self.index_lookup[level][0][:,2]]+\
                # 0.5*input_clone[:, (self.index_lookup[level][1][:,0]+i)%10, self.index_lookup[level][1][:,1],  self.index_lookup[level][1][:,2]] 
        result = input_tr.permute(0,1,4,2,3)
        result = result.view(result.size(0)*10,result.size(2),result.size(3),result.size(4))
        # del input_tr, idx_out, idx_in
