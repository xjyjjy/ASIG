...
def build_mat_paths():
    return ['/home/zwxionggroup/xiajy/dataset/arg_map_dataset_pano_level8/8_768_1024_60_80_180_0_arg_new_v2.mat']


def prepare_output_dirs(save_path):
    for dirname in ['pano_big', 'pano_ema_big']:
...
def save_outputs(outputs, to_pil_img, save_path, basename, ema):
    ...
    for output in outputs:
        ...
        for image_index in range(images.shape[0]):
            to_pil_img(images[image_index]).save(os.path.join(save_path, dirname, f'{basename}.png'))
...
def dm_worker(rank, tokenizer, tokenizer_2, text_list, args, save_path, ema=True):
    ...
    for _, filename in tqdm(enumerate(txt_files)):
        ...
        check_path = os.path.join(save_path, f'pano{"_ema_big" if ema else ""}/{basename}.png')
