...
def xyz2uv(xyz):
    ...
    v = - np.arctan2(y, c)
    # print("v", v.max(), v.min())
    # print("u", u, u.max(), u.min())
    return np.stack([u, v], axis=-1)
def xyz2uv_panorama(xyz):
    x, y, z = xyz[..., 0], xyz[..., 1], xyz[..., 2]
    u = np.arctan2(x, z)
    c = np.sqrt(x * x + z * z)
    # v = np.arctan2(y, c)
...
def remap(img, img_idx, cval=[0, 0, 0], method="linear"):
    ...
    if img.ndim == 2:
        ...
        bs = img.shape[0]
        # print("img_idx", img_idx.shape, img_idx.max(), img_idx.min())
        # print("img", img.shape)
        img_idx = torch.from_numpy(img_idx).unsqueeze(0).repeat(bs,1,1,1).to(img.device).float()
        x = F.grid_sample(img, img_idx.flip(-1), mode='bicubic', align_corners=False)
        # x = F.grid_sample(img, img_idx.flip(-1), mode='bicubic', align_corners=False)
    return x
...
def uv2img_idx(uv, erp_img):
    # print("erp_img", erp_img.shape)
    h, w = erp_img.shape[-2:]
    delta_w = 2 * np.pi / w
    delta_h = np.pi / h
    x = uv[..., 0] / delta_w + w / 2 
    y = uv[..., 1] / delta_h + h / 2 
    x = x * (x.max() - 1 ) / x.max() + 0.5
    y = y * (y.max() - 1) / y.max() + 0.5
    x = (x / w - 0.5) * 2
    y = (y / h - 0.5) * 2
    x = np.clip(x, -1, 1)
    y = np.clip(y, -1, 1)
